...
def main() -> None:
    ...
    # training split and persisted so analysis reuses the identical transform.
    ...
    if norm_p.exists():
        ...
        emb_mean, emb_std = compute_embedding_stats(paths)
        np.savez(norm_p, mean=emb_mean, std=emb_std)
        print(f"[sae] computed embedding norm over {len(paths)} files -> {norm_p.name}")
    ...
    for epoch in range(1, (args.epochs or config.get("sae", {}).get("epochs", 1)) + 1):
        ...
        while file_idx < n_paths:
            ...
            if global_step % 200 == 0:
                ...
                save_checkpoint(ckpt, sae_ckpt_dir, epoch=epoch, is_best=is_best)
    ...
    ckpt_path = save_checkpoint(ckpt, sae_ckpt_dir, epoch=epoch, is_best=True)
